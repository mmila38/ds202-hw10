...
driver$SEX = as.factor(driver$SEX)
levels(driver$SEX)
# According to documentation, 1 = MALE, 2 = FEMALE, 8 = NOT REPORTED, 9 = UNKNOWN
levels(driver$SEX) = c("Male", "Female", "Not Reported", "Unknown")
levels(driver$SEX)

## Join accident and driver table
driver_accident = inner_join(driver, accident.temp, by = "ST_CASE")

## Tally the number of accidents by day of the week (DAY_WEEK), hour of the day (HOUR) and gender (SEX). Visualize the results!

driver_accident_cleaned = driver_accident %>% filter(HOUR.x <= 24, (SEX == "Male" | SEX == "Female"))
driver_accident_cleaned %>% ggplot(aes(x = HOUR.x, fill = SEX)) + 
  geom_bar() + facet_wrap(~DAY_WEEK) + 
  labs(x = "Hour of the Day", y = "Count", title = "Accidents by Gender")
```
